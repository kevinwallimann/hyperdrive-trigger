...
class TimeSensorIntegrationTest extends FlatSpec with Matchers with BeforeAndAfterAll with BeforeAndAfterEach with RepositoryTestBase {
  ...
  private val workflowHistory: WorkflowHistoryRepositoryImpl = new WorkflowHistoryRepositoryImpl {
    override val profile = h2Profile
  }
  private val workflowRepository: WorkflowRepositoryImpl = new WorkflowRepositoryImpl(workflowHistory) {
    override val profile = h2Profile
  }
